...
import { Flex, Checkbox, Text, Card, Avatar, IconButton, Badge, Heading, Link, Button, Container, Grid } from "@radix-ui/themes";
...
const VenuesPage = () => {
  ...
  return (
    <StyledContainer>
      <Heading size="8" mb="4" align="center">Our Venues</Heading>
      <FilterSection direction="column">
        <Text size="5" mb="2">Filter:</Text>
          ...
          {filterOptions.map((option) => (
            <Flex key={option} align="center">
              <Checkbox
                ...
                id={option}
              />
              <Text as="label" ml="1" htmlFor={option}>{option}</Text>
            </Flex>
          ))}
        </Flex>
        ...
        {filteredVenues.map((venue: Venue, index: number) => (
          <VenueCard key={index}>
            <Flex direction="column" gap="2" height={"100%"}>
              ...
              <Text size="2" color="gray" >{venue.description}</Text>
              <Flex align="center" gap="2">
                <Link href={venue.mapLink} target="_blank" rel="noopener noreferrer">
